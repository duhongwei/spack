...
module.exports = {
  makeHotConfig: function (specialConifg) {
    const logger = specialConifg.logger
    const configHotFile='config-hot.js'
    debug('read base config')
    const baseConfig = require(`./${configHotFile}`)
    ...
    debug(`read env config folder ${f}`)
    let clientConfig = require(resolve(f,configHotFile))
    let config = {
