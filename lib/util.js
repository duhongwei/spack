...
function isFont(file) {
  let ext = file.split('.').reverse()[0]
  return /^(eot|ttf|woff|woff2|etf)$/.test(ext)
}
function isHtmlOrTpl(file) {
  let ext = file.split('.').reverse()[0]
  return /^(html|tpl)$/.test(ext)
}
 ...
 * @param {function} cb 
 */
function replaceImage(content, cb) {
  function testWebUrl(picUrl) {
  ...
  /*image as background-image or font*/
  content = content.replace(/\burl\([\s'"]*([^)]+?\.[^#?)\s'"]+)[^)]*\)/g, (match, picUrl) => {

    if (testWebUrl(picUrl)) {
      return match
    }

    if (!isImage(picUrl) && !isFont(picUrl)) {
      return match
    }
    picUrl = cb(picUrl)
    return `url(${picUrl})`
  ...
  /*<img src=''> 图片名称排除+号是排除 js 拼接字符串的情况 +?非贪婪，否则会把后面的 \/ 也匹配进来 */
  content = content.replace(/<img([^>]+?)\/?>/g, function (match, img) {
    //\bsrc不行，因为可以匹配 :src
    var replaced = img.replace(/\s+src\s*=['"\s]*([^'"+]+\.\w{3,4})['"\s]*/g, (match, picUrl) => {
      if (testWebUrl(picUrl)) {
        return match
      }
  ...
  /*js中包含的图片 */
  content = content.replace(/\.src\s*=\s*['"]([^'"+]+\.\w{3,4})['"]/g, (match, picUrl) => {
    if (testWebUrl(picUrl)) {
      return match
    }
