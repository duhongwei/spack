...
function isImage(file) {
  ...
  return /^(gif|jpg|png|jpeg|svg|ico)$/.test(ext)
}
...
function isText(file) {
  let ext = file.split('.').reverse()[0]
  return /^(js|css|html|htm|tpl|vue|jsx|wxss|wxml|json|wxs|less|ts)$/.test(ext)
}
///a.min.js
function isMin(file) {
  let f = file.split('.')
...
function image2base64(file, data) {

  let s = Buffer(data).toString('base64');
  return `data:image/${extname(file).substr(1)};base64,${s}`
}
