...
class ToAmd {
  constructor({ importInfo, exportInfo, code, sourcePath, files }, key, { isSimple = true } = {}) {
  ...
  _importToToken() {
    ...
    for (let item of this.importInfo) {
      if (item.type === 'css') {
        this.deps.push(pathToKey(item.file, this.key, this.sourcePath, this.files))
        continue
      }
      if (item.type === 'less') {
        this.deps.push(pathToKey(`${item.file}.css`, this.key, this.sourcePath, this.files))
        continue
      }

      let thisToken = getToken()
      if (item.type === 'ts') {
        deps.push(pathToKey(`${item.file}.js`, this.key, this.sourcePath, this.files))
      }
      else {
        deps.push(pathToKey(item.file, this.key, this.sourcePath, this.files))
      }
...
function pathToKey(path, key, sourcePath, files) {
  if (/^http/.test(path)) {
