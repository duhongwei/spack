...
function prepare({ keys, source, logger, alias }) {
  ...
  for (const key of keys) {
    ...
    if (nodeKey in alias) {
      ...
      /* 直接改造umd 影响太大，先不弄了
            let key = `node/${nodeKey}.js`;
            code = code.replace(/define\(\[([^\]]+)]/, (m, $1) => {
              let keys = $1.split(',').map(item => item.replace(/['\s"]/g, '')).map(item => {
                if (item === 'exports') return item
                else {
                  return `node/${item}.js`
                }
              }).map(item => `"${item}"`).join(',')
              return `define("${key}",[${keys}]`
            }) */
