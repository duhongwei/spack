...
function getEntry(file) {
  return file.replace('.html', '.js').split(/\/|\\/).join('/')
}
module.exports = class {
  constructor(config) {
    ...
    //因为路径线可能是 \ 或 / 
    ignoreReg.unshift(/\/include\/[^.]+\.html$/)
    ...
    if (config.cdn) {
      ...
      spack.cdn = new Cdn(spack.destination(), spack.publicPath, spack.prePath)
